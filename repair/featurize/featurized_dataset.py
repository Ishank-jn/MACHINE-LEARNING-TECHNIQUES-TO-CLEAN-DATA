...
class FeaturizedDataset:
    ...
    def generate_weak_labels(self):
        ...
        #WHERE (sample_ind = 0 AND t2._cid_ IS NULL) AND weak_label != '{null_repr}' AND (t2._cid_ is NULL OR t1.fixed != {cell_status});
        query = """
        ...
        WHERE ((sample_ind = 1 AND t2._cid_ IS NULL) OR (t2._cid_ IS NOT NULL)) AND weak_label != '{null_repr}' AND (t2._cid_ is NULL OR t1.fixed != {cell_status});
